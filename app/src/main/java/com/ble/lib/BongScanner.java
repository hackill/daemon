...
public class BongScanner {
    ...
    private BluetoothDevice getDeviceConnected() {
        BluetoothManager bluetoothManager = (BluetoothManager) BongApp.getBongComponent().getBongApp().getSystemService(Context.BLUETOOTH_SERVICE);

        List<BluetoothDevice> connectedDevices = bluetoothManager.getConnectedDevices(BluetoothProfile.GATT);

        for (BluetoothDevice device : connectedDevices) {

            if (TextUtils.equals(device.getName(), mBong.toString())) {
                return device;
            }
...
//                Log.i(TAG, "decode " + deviceName);
            }
            // 正常的搜索
            if (TextUtils.equals(deviceName, mBong.toString())) {
