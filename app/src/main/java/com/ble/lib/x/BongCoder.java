...
import cn.ginshell.bong.utils.DateUtil;
...
public class BongCoder {
    ...
    public static byte[] encodeBong3Screen(ContentConfigModel contentConfig) {
        ...
        boolean sport = contentConfig.isShowSport();

        String string = String.format(Locale.ENGLISH, "290000001A%02X%02X%02X%02X%02X%02X"
                , step ? 1 : 0
                , distance ? 1 : 0
                , cal ? 1 : 0
                , weather ? 1 : 0
     ...
     * @return
     */
    public static byte[] encodeScreenStyle(boolean vertical, boolean auto, boolean right, boolean highlight, boolean wrist) {


        String str = String.format(Locale.ENGLISH, "2900000013%02X%02X%02X%02X%02X",
                vertical ? 1 : 0,
                auto ? 1 : 0,
                right ? 1 : 0,
                highlight ? 1 : 0,
    ...
    /**
     * 指令 2900000019+能量（4个字节）+步数（4个字节）+距离（字节）
     *
     * @param energy   能量 单位 焦耳
     * @param steps    步数 单位 步
    ...
    /**
     * 指令 2900000019+能量（4个字节）+步数（4个字节）+距离（字节）+（睡眠时间 2个字节 单位分钟）
     *
     * @param energy   能量 单位 焦耳
     * @param steps    步数 单位 步
     * @param distance 距离 单位米
     * @return
     */
    public static byte[] encodePostSummery(int energy, int steps, int distance, int sleep) {
        String str = String.format(Locale.ENGLISH, "2900000019%08X%08X%08X%04X", energy, steps, distance, sleep);
...
//    }
     ...
     * @return
     */
    public static byte[] encodeReadManualSportByTime(long startTime, long endTime) {
        String cmd = "2000000016" + BongHexUtils.getStrTimeForHex(startTime) + BongHexUtils.getStrTimeForHex(endTime);
        return BongUtil.hexStringToBytes(cmd);
    }
...
//    }
    ...
    public static byte[] encodeReadBongFitVersion() {
        return BongUtil.hexStringToBytes("25000000F8");
    }
     ...
     * @return
     */
    public static String encodeXHeartSwitch(boolean open, int routine) {
        String dif = open ? "01" : "00";
        String min = String.format("%04x", routine);//例子:16进制输出20，宽8位，右对齐，左补0，结果为00000014
        String second = String.format("%02x", 20);//1个字节
        return "2600000054" + dif + min + second;
    }
    ...
    /**
     * send: 0x41 00 00 05 + account_id(3bytes 高位在前低位在后)
     *
     * @return
     */
    public static byte[] cmdFitCurrentAccount(int userId) {
        String cmd = "41000005%06X";
        return BongUtil.hexStringToBytes(String.format(cmd, userId));
     ...
     * @return
     */
    public static byte[] encodeFitTimeCmd() {
        String timeCmd = "11000001%016X";
        return BongUtil.hexStringToBytes(String.format(timeCmd, System.currentTimeMillis() / 1000));
    }
     ...
     * @return
     */
    public static byte[] encodeGetMarking() {
        String cmd = "43000001";
        return BongUtil.hexStringToBytes(cmd);
    }
     ...
     * @return
     */
    public static byte[] encodeFitRecovery() {
        return BongUtil.hexStringToBytes("44000101");
    }


    public static byte[] encodeAddFitAccount(FitAccount fitUser, float weightV) {
        int age = DateUtil.getCurrentYear() - fitUser.getBirthday();
        if (age < 0) age = 0;
        int weight = (int) (weightV * 100);
        return encodeAddFitAccount(fitUser.getParentUserId(), fitUser.getSubId(), fitUser.getGender(), age, fitUser.getHeight(), weight);
    }

    /**
     * send: 0x40 00 00 04 + account_id(3bytes 高位在前低位在后) + user_id(1byte) + sex(1byte) + age(1byte) + height(1byte) + weight(2bytes 高位在前低位在后)
     * 如果没有体重信息，则填充0x0000
     * response: 成功返回success，失败返回：0x40 00 00 04|0x80 96个坑存满无剩余空间需要删除已有成员
     * Sex：1:girl, 0:boy
     *
     * @return
     */
    private static byte[] encodeAddFitAccount(int userId, int subId, int sex, int age, int height, int weight) {
        String cmd = "40000004%06X%02X%02X%02X%02X%04X";
//        String cmd = "40000004   %06X   %02X %02X %02X %02X        %04X";
        //[(0x)       40000004-00-00-00 -00  -01  -1A  -AA        -1770]
        return BongUtil.hexStringToBytes(String.format(cmd, userId, subId, sex, age, height, weight));
    }

    /**
     * send: 0x40 00 00 02 + account_id(3bytes 高位在前低位在后)
     * response: 成功返回：success，失败返回: 0x40 00 00 02|0x80 没有该主账号
     *
     * @param userId
     * @return
     */
    public static byte[] encodeUnbindFit(int userId) {
        String cmd = "40000002%06X";
        return BongUtil.hexStringToBytes(String.format(cmd, userId));
    }

    public static byte[] encodeDeleteFitAccount(int userId, int subId) {
        String cmd = "40000003%06X%02X";
        return BongUtil.hexStringToBytes(String.format(cmd, userId, subId));
    }

    /**
     * send: 0x40 00 00 05 + account_id(3bytes 高位在前低位在后) + len(1byte) + user_id(1 byte) + weight(2bytes 高位在前低位在后) + ... + user_id(1 byte) + weight(2bytes 高位在前低位在后)
     * <p>
     * (0x) 40-00-00-05-00-91-CA-03-0E-1C-E8]
     *
     * @return
     */
    public static byte[] encodeUpdateFitUserInfo(FitAccount fitAccount, float weight) {
        if (fitAccount != null) {

            String cmd = "40000005%06X%02X%02X%04X";

            int weightV = (int) (weight * 100);

            return BongUtil.hexStringToBytes(String.format(cmd, fitAccount.getParentUserId(), 3, fitAccount.getSubId(), weightV));
        }
        return null;
    }

    /**
     * send: 0x42 00 00 01 + timestamp_start(4bytes 低32位时间戳) + timestamp_end(4bytes 低32位时间戳) + account_id(3bytes 高位在前低位在后)
     * response: 20字节称数据格式（见图1） * n个历史数据
     *
     * @return
     */
    public static byte[] readFitHistoryData(int userId, long startTime, long endTime) {
        String cmd = "42000001%08X%08X%06X";
        return BongUtil.hexStringToBytes(String.format(cmd, startTime, endTime, userId));
    }

    public static byte[] encodeUpdateFitUserInfo(List<FitAccount> fitAccountList, List<FitWeightModel> modelList) {
        if (fitAccountList != null && modelList != null &&
                fitAccountList.size() <= 4 && fitAccountList.size() == modelList.size()) {

            String cmd = "40000005%06X%02X";
            String cmdSux = "%02X%04X";

            int length = fitAccountList.size() * 3;
            StringBuilder stringBuffer = new StringBuilder();

            for (int i = 0; i < fitAccountList.size(); i++) {
                FitAccount fitAccount = fitAccountList.get(i);

                int subId = fitAccount.getSubId();
                int weight = (int) (modelList.get(i).getWeight() * 100);
                stringBuffer.append(String.format(cmdSux, subId, weight));
            }

            String cmd1 = String.format(cmd, fitAccountList.get(0).getParentUserId(), length);
            return BongUtil.hexStringToBytes(cmd1 + stringBuffer.toString());

        } else {
            throw new RuntimeException("fitAccountList is not legal");
        }
    }

    public static byte[] encodeReadAllFitAccount(int userId) {
        String cmd = "40000001%06X";
        return BongUtil.hexStringToBytes(String.format(cmd, userId));
    }

    public static byte[] ensureFitCome() {
        return BongUtil.hexStringToBytes("41000101");
    }

    /**
     * send: 0x41 00 00 06 + account_id(3bytes 高位在前低位在后) + user_id(1byte) + sex(1byte) + age(1byte) + height(1byte)
     * reponse: success
     *
     * @return
     */
    public static byte[] ensureBindFit(User user) {

        int gender = TextUtils.equals(user.getGender(), "1") ? 0 : 1;
        int age = DateUtil.getCurrentYear() - user.getBirthday();

        return ensureBindFit(user.getId(), 0, gender, age, user.getHeight());
    }

    public static byte[] ensureBindFit(int userId, int subId, int sex, int age, int height) {
        String cmd = "41000006%06X%02X%02X%02X%02X";
        return BongUtil.hexStringToBytes(String.format(cmd, userId, subId, sex, age, height));
    }

    /**
     * 发送：0x29 00 00 00 21 + 表盘序号（1字节）
     *
     * @param position
     * @return
     */
    public static byte[] encodeSetB3Plate(int position) {
        String cmd = "2900000021%02X";
        return BongUtil.hexStringToBytes(String.format(cmd, position));
    }

    /**
     * 0x29 00 00 00 22 + (平衡目标，4字节，单位焦）00 00 00 00 + （睡眠目标，4字节，分钟）00 00 00 00
     *
     * @param kcal 千焦
     * @return
     */
    public static byte[] encodeSetDeviceGoal(int kcal, int sleepMins) {
        kcal = kcal * 1000;
        String cmd = "2900000022%08X%08X";
        return BongUtil.hexStringToBytes(String.format(cmd, kcal, sleepMins));
    }
     ...
     * @return
     */
    public static byte[] encodeShowBind() {
        return BongUtil.hexStringToBytes("2900000017");
    }
     ...
     * @return
     */
    public static byte[] encodeShowBindSuccess() {
        return BongUtil.hexStringToBytes("2900000018");
    }
}
